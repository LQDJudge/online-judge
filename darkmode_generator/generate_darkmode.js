...
async function generateDarkModeCSS(url = DEFAULT_URL) {
    console.log(`🌙 Generating dark mode CSS for ${url}...`);
    ...
    try {
        ...
        console.log('🎨 Enabling DarkReader and generating CSS...');
...
`;
        ...
        console.log(`✅ Dark mode CSS saved to: ${OUTPUT_FILE}`);
        console.log(`📏 CSS size: ${css.length.toLocaleString()} characters`);
        ...
        console.log(`🗜️  Minified CSS saved to: darkmode.min.css`);
        console.log(`📏 Minified size: ${minifiedCSS.length.toLocaleString()} characters`);
        ...
        console.error(`❌ Error generating dark mode CSS: ${error.message}`);
        process.exit(1);
    } finally {
        await browser.close();
    }
...
async function main() {
    ...
    try {
        ...
        if (!response.ok) {
            console.warn(`⚠️  Warning: Server returned status ${response.status}`);
        }
    } catch (error) {
        console.warn(`⚠️  Warning: Cannot connect to ${url}`);
    ...
    try {
        ...
        console.log('\n✨ Done! Dark mode CSS generated successfully.');
    } catch (error) {
