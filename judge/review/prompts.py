...
- Danh sách các bài nộp tham chiếu, mỗi bài gồm: id, ngôn ngữ, mã nguồn, điểm đạt được (case_points/case_total), thời gian chạy.

Hãy đánh giá tập này theo các tiêu chí sau:

1. Suy luận chế độ bài: "OI" nếu đề khai báo subtask với %, "ICPC" nếu không.
2. Với mỗi bài nộp, xác định:
   - role: "main_ac" (giải tối ưu, kỳ vọng AC đầy đủ), "subtask_K" (giải nhắm subtask K, K là số 1-indexed), "brute_force" (giải sơ khai, complexity cao hơn tối ưu), hoặc "unclear".
...
3. Đánh giá tổng thể (issues), bao gồm các trường hợp:
   ...
   - Bài brute_force lại đạt full AC (suggesting tests are weak).
   - Khai báo complexity sai khớp với code (nếu tác giả có đặt claimed_complexity).
...
Trả lời CHỈ ở định dạng JSON HỢP LỆ:
{
  "mode": "OI" | "ICPC",
  "submissions": [
    {"submission_id": N, "role": "main_ac" | "subtask_K" | "brute_force" | "unclear", "complexity_observed": "O(...)", "correctness": "correct" | "wrong" | "unclear", "achieved_pct": 100.0, "note": "ghi chú ngắn bằng tiếng Việt"}
  ],
